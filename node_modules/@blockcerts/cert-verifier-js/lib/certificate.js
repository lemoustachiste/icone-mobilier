...
var _index = _interopRequireDefault(require("./parsers/index"));
...
class Certificate {
  ...
  async parseJson(certificateDefinition) {
    const parsedCertificate = await (0, _index.default)(certificateDefinition);
  ...
  _setOptions(options) {
    ...
    if (options.didResolverUrl) {
      _domain.default.did.didResolver.url = options.didResolverUrl;
    }
  ...
  _setProperties({
    ...
    this.verificationSteps = _domain.default.certificates.getVerificationMap(chain, version, !!this.issuer.didDocument);
